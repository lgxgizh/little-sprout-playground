...
const modelCatalog = {
  image: [
    ...
    { id: 'local-image', name: '本地图片库', note: '使用 public/assets 中的素材' },
  ],
  voice: [
    { id: 'browser-speech', name: '浏览器语音', note: '无需密钥 · 当前默认' },
    { id: 'gpt-4o-mini-tts', name: 'OpenAI TTS', note: '自然中文 · 需配置 API' },
    { id: 'local-audio', name: '本地音频', note: '播放题目音频文件' },
  ],
  vocab: [
    { id: 'adaptive-picture', name: '图片自适应测试', note: '按答题表现动态调整' },
    { id: 'gpt-4o-mini', name: 'GPT-4o mini', note: '生成个性化题目 · 需配置 API' },
    { id: 'local-question-bank', name: '本地题库', note: '使用你提供的固定题目' },
  ],
};

const savedModels = (() => {
  try { return JSON.parse(localStorage.getItem('little-fun-models') || '{}'); } catch { return {}; }
})();
const models = {
...
function modelName(type) {
  return modelCatalog[type].find(item => item.id === models[type])?.name || models[type];
}

function saveModels() {
  try { localStorage.setItem('little-fun-models', JSON.stringify(models)); } catch { /* 隐私模式下仍可继续使用当前会话设置 */ }
}
...
function bindEvents() {
  ...
  document.querySelector('#voicePrompt')?.addEventListener('click', () => { if (models.voice === 'local-audio') showToast('请在 public/assets/audio 中放入题目音频'); else speak('小栗子想找一颗蓝色的水果，你能帮帮它吗？'); });
  ...
  document.querySelectorAll('[data-model]').forEach(select => select.addEventListener('change', () => { models[select.dataset.model] = select.value; saveModels(); const tip = document.querySelector('.config-tip'); if (tip) tip.innerHTML = `当前语音：<b>${modelName('voice')}</b> · 当前题目：<b>${modelName('vocab')}</b>`; const chip = document.querySelector('.model-chip b'); if (chip) chip.textContent = modelName('vocab'); const imageBadge = document.querySelector('.active-model'); if (imageBadge) imageBadge.textContent = `图片：${modelName('image')}`; }));
  document.querySelector('#resetModels')?.addEventListener('click', () => { models.image = 'gpt-image-1'; models.voice = 'browser-speech'; models.vocab = 'adaptive-picture'; saveModels(); render(); });
